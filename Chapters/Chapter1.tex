...
\subsection{Definición de chatbot}
...
\subsection{Chatbots como solución eficiente}
...
\begin{itemize}
    \item Disponibilidad permanente: los chatbots están disponibles en todo momento, lo que les permite a las empresas atender consultas de sus clientes a cualquier hora del día.
...
\begin{itemize}
    \item Desarrollo de interfaces de usuario avanzadas, por fuera de las básicas necesarias para probar el \textit{pipeline}.
...
Los chatbots se han integrado en numerosos aspectos de la vida diaria y en diversos canales de comunicación. Se encuentran presentes en redes sociales como Facebook e Instagram, donde facilitan la interacción con empresas \cite{facebook-workplace-bots}. Además, muchas organizaciones y entidades gubernamentales, han implementado chatbots para mejorar la atención al ciudadano. Plataformas como IBM Watson \cite{ibm-watson}, Google Dialogflow \cite{google-dialogflow} y Microsoft Bot Framework \cite{microsoft-botframework} permiten a las empresas crear chatbots personalizados con gran flexibilidad.

\subsection{Recursos para chatbots}

\subsubsection{Servicios basados en APIs}
Los servicios basados en API (por sus siglas en inglés, \textit{Application Programming Interface}) simplifican la implementación de chatbots al proporcionar interfaces estandarizadas para el acceso a funcionalidades avanzadas. Estos servicios permiten una mayor personalización y flexibilidad en el diseño de chatbots, que facilitan la integración con otros sistemas y plataformas \cite{openai-api}. La utilización de APIs permite a las empresas adaptar las soluciones a sus necesidades específicas sin necesidad de desarrollar toda la infraestructura desde cero.


\newpage
\subsubsection{Modelos locales}
Los modelos locales como Llama 3.1 \cite{meta-ai-llama}, Phi 3, Gemma \cite{google-deepmind-gemma} y Mistral \cite{mistral-ai} son soluciones robustas que operan sin depender de servicios externos. Cada uno de estos ofrece características y ventajas específicas, como mayor control sobre los datos y la posibilidad de operar en entornos con restricciones de privacidad. Su uso es particularmente valioso cuando se requiere un alto nivel de personalización y seguridad en la gestión de datos.


\subsection{Tendencias actuales}
Cada vez más empresas optan por utilizar servicios basados en API, que han hecho más accesible y económico el despliegue de chatbots sofisticados. Estas herramientas simplifican la implementación y permiten la creación rápida de soluciones adaptadas a las necesidades específicas de cada empresa o entidad. Sin embargo, aunque los modelos locales son valiosos por su robustez y capacidad para operar sin depender de servicios externos, su utilidad es particularmente destacada cuando se requiere un mayor control o confidencialidad en la gestión de datos, en contraste con las soluciones basadas en la nube \cite{ibm-on-premise-cloud}.


\subsection{Consideraciones éticas en el uso de chatbots}

El desarrollo e implementación de chatbots inteligentes plantea no solo desafíos técnicos, sino también importantes cuestiones éticas, especialmente cuando se emplean en contextos sensibles como la atención al cliente, la gestión de datos o la interacción con usuarios vulnerables. Estas consideraciones resultan esenciales para garantizar que la automatización de las interacciones no comprometa la privacidad, la transparencia ni la equidad \cite{partnership-on-ai}.

\subsubsection{Privacidad y manejo de datos}

Una de las principales preocupaciones éticas se relaciona con la protección de los datos personales. Al interactuar con usuarios, los chatbots pueden recolectar información sensible de manera no intencionada. Por ello, es fundamental que estos sistemas se diseñen con el objetivo de limitar el almacenamiento innecesario de datos y garantizar su anonimato \cite{ico-ai-data-protection}.

\subsubsection{Limitaciones temáticas y control de contenido}

Otro aspecto relevante es el control sobre el tipo de consultas que el chatbot puede atender. En entornos públicos o semiabiertos, es posible que los usuarios formulen preguntas relacionadas con temas delicados, como política, religión o salud. La generación automatizada de respuestas en estos ámbitos puede derivar en desinformación o respuestas inapropiadas. Por este motivo, se incorporan mecanismos de filtrado, denominados \emph{guardrails}, que permiten detectar y bloquear consultas fuera del dominio permitido, lo que contribuye a la seguridad y alineación ética del sistema \cite{nvidia-guardrails-ai}.

\newpage
\subsubsection{Transparencia y límites de automatización}

Es fundamental que los usuarios sean conscientes de que interactúan con un sistema automatizado y no con una persona. Esta transparencia evita la generación de falsas expectativas y reduce el riesgo de que se confíe en el chatbot para la toma de decisiones críticas. Asimismo, el sistema debe contar con mecanismos que le permitan reconocer sus propias limitaciones y, en caso de no disponer de información contextual suficiente, abstenerse de responder o derivar la consulta a un operador humano, en línea con los principios de una IA responsable establecidos en las Recomendaciones para el uso de Inteligencia Artificial del Gobierno de Argentina \cite{argentina-enia} y otras directrices internacionales.

\subsubsection{Equidad y sesgos}

Los modelos de lenguaje empleados por los chatbots pueden reproducir sesgos presentes en los datos de entrenamiento. Si no se abordan de manera adecuada, dichos sesgos pueden perpetuar discriminaciones o estereotipos. Por ello, resulta esencial realizar un análisis exhaustivo de las fuentes de datos utilizadas en el entrenamiento, así como aplicar técnicas de mitigación. Entre estas se incluyen la revisión crítica de los conjuntos de datos, la identificación sistemática de sesgos y la implementación de estrategias orientadas a su corrección, conforme a estudios sobre sesgos algorítmicos y representación social en modelos de lenguaje generativo \cite{fundar-sesgos}. Asimismo, es fundamental efectuar pruebas continuas que permitan verificar que el sistema se mantenga libre de sesgos a lo largo del tiempo.
